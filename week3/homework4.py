...
def calculate_parentheses_all_set(index, tokens):
  next_index = index
  while next_index < len(tokens):
    tokens, next_index, _ = calculate_parentheses_per_set(next_index, tokens)
  return tokens

def calculate_parentheses_per_set(index, tokens):
  """
  ...
  Args:
      index (int): index of the left parentheses that starts the computation including
      tokens (list): a list of tokenized tokens. Each element is a dictionary with
                     ...
                     ex) [{'type' : 'PLUS'}, {'type': 'PARENTHESES_LEFT'}, {'type': 'NUMBER', 'number' : 15}, {'type': 'PLUS'}, {'type': 'NUMBER', 'number' : 3}, {'type': 'PARENTHESES_RIGHT'}]
  ...
  while index < len(tokens):
    ...
    if tokens[index]['type'] == 'PARENTHESES_RIGHT':
      ...
      if len(parentheses_left_index_stack) == 0:
        return tokens, index + 1, result
